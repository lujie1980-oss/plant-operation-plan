# OTD 本体 ↔ Plant Operation Plan 映射基线（M1）

> **版本：** M1 设计基线（Epic 0 / Task 0.1）  
...
共 **27** 行，覆盖 M1 主计划本体 PoC 所需的主数据、供需、时段、产能与 Session 边界。
...
| **Period** | `period.Period` | —（内存，M1 固定 28 日桶） | `TimeSlot`（粒度对照，非 1:1） | — | 实现 |
...
| **Operation**（工序实例） | — | — | `OrderAllocation`（S04）/ `OperationAssignment`（S05） | `MasterPlanPlanningPreviewAllocationDto` / `DetailScheduleOperationDto` | 暂缓 |
...
| **MasterPlanOntologySession** | —（`scenario.planning`） | —（内存，8h TTL） | — | `MasterPlanSessionDto` / `MasterPlanSessionSimulateResultDto` | 实现 |
| **SchedulingSession** | —（`scenario.planning`，S05 细排） | —（内存） | `DetailSchedule` | `ScheduleSessionDto` | 映射 |
...
M1 实现 **create → simulate**；方括号内为 M2 能力。
...
并行参考（S05，已有）：
  ...
  M1 Epic 1：SchedulingSessionStore.require(sessionId, workspaceId) 硬隔离
```
...
| M1 本体（待建） | `src/main/java/com/plantops/ontology/` |
| M1 ROL（待建） | `src/main/java/com/plantops/rol/` |

---

*文档随 M1 实施更新；类名以 `src/main/java` 为准。Epic 0 Task 0.1 交付物 — 团队评审后在本节追加评审链接。*
